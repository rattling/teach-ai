...
def decision_tree_main():
    ...
    sns.boxplot(
        x="creditworthy", y="months_employed", data=data, ax=axes[0, 2]
    )
    sns.boxplot(
        x="creditworthy", y="avg_credit_card_bal", data=data, ax=axes[1, 0]
    )
    sns.boxplot(
        x="creditworthy", y="months_with_bank", data=data, ax=axes[1, 1]
    )
    ...
    data_melted = pd.melt(
        data, id_vars=["creditworthy"], value_vars=education_levels
    )
    ...
    prediction_label = (
        "Creditworthy" if prediction == 1 else "Not Creditworthy"
    )
